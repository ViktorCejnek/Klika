...
typedef struct{
  ...
  uint16_t  CustomChar_CommandHdle;                  /**< char_command handle */
  uint16_t  CustomChar_StatusHdle;                  /**< char_status handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeChar_Command = 2;
uint16_t SizeChar_Status = 1;
...
#define COPY_PHONE_COMMUNICATION_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x69,0x69,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CHAR_COMMAND_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_CHAR_STATUS_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomChar_StatusHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Indication)):
                ...
                /* USER CODE END CUSTOM_STM_Service_1_Char_2_attribute_modified */
                Notification.Custom_Evt_Opcode = CUSTOM_STM_CHAR_STATUS_INDICATE_DISABLED_EVT;
              ...
              Notification.Custom_Evt_Opcode = CUSTOM_STM_CHAR_STATUS_INDICATE_ENABLED_EVT;
              Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomChar_CommandHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            /* USER CODE BEGIN CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */

            /* USER CODE END CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomChar_CommandHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
        ...
        case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:
          ...
          if (write_perm_req->Attribute_Handle == (CustomContext.CustomChar_CommandHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *                                2 for char_command +
   *                                2 for char_status +
   *                                1 for char_status configuration descriptor +
   *                              = 6
   *
  ...
  COPY_PHONE_COMMUNICATION_UUID(uuid.Char_UUID_128);
   ...
   *  char_command
   */
  COPY_CHAR_COMMAND_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomPhone_CommHdle,
                          ...
                          &(CustomContext.CustomChar_CommandHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CHAR_COMMAND, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CHAR_COMMAND \n\r");
  }
   ...
   *  char_status
   */
  COPY_CHAR_STATUS_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomPhone_CommHdle,
                          UUID_TYPE_128, &uuid,
                          SizeChar_Status,
                          CHAR_PROP_INDICATE,
                          ...
                          &(CustomContext.CustomChar_StatusHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CHAR_STATUS, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CHAR_STATUS \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHAR_COMMAND:
      ret = aci_gatt_update_char_value(CustomContext.CustomPhone_CommHdle,
                                       CustomContext.CustomChar_CommandHdle,
                                       0, /* charValOffset */
                                       SizeChar_Command, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CHAR_COMMAND command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CHAR_COMMAND command\n\r");
      }
    ...
    case CUSTOM_STM_CHAR_STATUS:
      ret = aci_gatt_update_char_value(CustomContext.CustomPhone_CommHdle,
                                       CustomContext.CustomChar_StatusHdle,
                                       0, /* charValOffset */
                                       SizeChar_Status, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CHAR_STATUS command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CHAR_STATUS command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHAR_COMMAND:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomPhone_CommHdle, CustomContext.CustomChar_CommandHdle, SizeChar_Command, pPayload);
    ...
    case CUSTOM_STM_CHAR_STATUS:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomPhone_CommHdle, CustomContext.CustomChar_StatusHdle, SizeChar_Status, pPayload);
